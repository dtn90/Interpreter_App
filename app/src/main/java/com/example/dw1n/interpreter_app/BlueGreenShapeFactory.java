 ...
 */
public class BlueGreenShapeFactory extends ShapeFactory{
    @Override
    public Shape getCircle(Context context, int x_coord, int y_coord, int radius) {
        return new BlueGreenCircle(context, x_coord, y_coord, radius);
    }

    @Override
    public Shape getRectangle(Context context, int x1_coord,
                              int y1_coord, int x2_coord, int y2_coord) {
        return new BlueGreenRectangle(context, x1_coord, y1_coord, x2_coord, y2_coord);
    }
}
